...
The scientific companion project [ActionABI](../actionabi/README.md) attacks the same problem from the
...
0.000–0.005 floor — and the same policy-agnostic belief adapters restore both to their oracle ceilings.
...
section (git history: `docs/positioning_litcheck.md`).
...
with the exact package environment recorded by the local lock file.
...
| `export_labeled_traces.py` | Exports labeled real-sim ManiSkill traces (ground-truth contracts) for supervised ActionABI evaluation. | `../../actionabi/reports/labeled_sim_traces.md` |
...
red-team and a 2026-07-21 literature pass reach these findings; full dated matrix and primary-source
links in git history (`docs/positioning_litcheck.md`, `docs/weakness_sota_recon.md`).
...
**C++ CPU-fusion anchor.** The ~13× CPU-over-32-thread-torch win in the many-environment regime is
...
`reports/cpp_fusion.md` and ActionABI's [Python bindings](../actionabi/README.md#python-bindings-pybind11-fusion-backend).
